...
public class Cliente_RestHeart_API_1 implements Serializable {
    ...
    public String buscar_adjunto(String id_adjunto) {
        String resultado = "";
        
        try {
            WebTarget webTarget = this.client.target(BASE_URI).path("procesos_titulares.files/" + id_adjunto);
    ...
    public String eliminar_adjunto(String id_adjunto) {
        String resultado = "";
        try {
            WebTarget webTarget = this.client.target(BASE_URI).path("procesos_titulares.files/" + id_adjunto);
            Invocation.Builder invocationBuilder =  webTarget.request(MediaType.APPLICATION_JSON);
            ...
            resultado = ex.toString();
            System.out.println("1,ERROR (" + this.getClass().getName() + " - promocion_docente_docente):" + ex.toString());
        }
